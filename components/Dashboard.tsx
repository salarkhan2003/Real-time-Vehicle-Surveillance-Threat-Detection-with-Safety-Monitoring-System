...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const globalThreat = isEmergencyAlarm ? ThreatLevel.CRITICAL : 
                       closestDistance < 2.5 ? ThreatLevel.HIGH : ThreatLevel.LOW;

  const threatColor = globalThreat === ThreatLevel.CRITICAL ? 'bg-red-500 shadow-[0_0_20px_#ef4444]' :
                      globalThreat === ThreatLevel.HIGH ? 'bg-orange-500' : 'bg-emerald-500';
    ...
    <div className={`h-screen w-screen bg-[#050505] flex flex-col p-4 space-y-4 select-none animate-fade-in relative transition-colors duration-200 ${isEmergencyAlarm ? 'bg-red-950/20' : ''}`}>
      ...
      {errorStatus && (
        <div className="absolute top-0 left-0 w-full z-[110] bg-red-600 text-white p-3 text-center text-[10px] font-black tracking-[0.2em] animate-pulse">
          SYSTEM_ALERT: {errorStatus} | MONITORING_THROTTLED
        </div>
      )}
      ...
      <div className="flex justify-between items-center bg-white/5 backdrop-blur-2xl p-4 rounded-3xl border border-white/10 shadow-xl z-10">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-[9px] text-white/40 uppercase font-black">Link: {isMonitoring ? (errorStatus ? 'Throttled' : 'Active') : 'Offline'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button 
            ...
            className="p-3 bg-white/5 hover:bg-white/10 rounded-2xl transition-all border border-white/5 text-white/60 active:scale-90"
          >
            {isFullscreen ? 'EXIT_FULL' : 'ENTER_FULL'}
          </button>

          <select 
            value={selectedCameraId}
            onChange={(e) => onCameraSelect(e.target.value)}
            className="bg-white/5 border border-white/10 text-[10px] font-bold uppercase rounded-xl px-4 py-2.5 text-white/80 outline-none"
          >
            {availableCameras.map(cam => (
              <option key={cam.deviceId} value={cam.deviceId} className="bg-slate-900">{cam.label || 'Optic 01'}</option>
            ))}
          </select>

          <button 
            onClick={onToggleMonitoring}
            className={`px-5 py-2.5 rounded-2xl font-black text-[10px] tracking-widest uppercase transition-all active:scale-95 ${isMonitoring ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-blue-600 text-white'}`}
          >
            {isMonitoring ? 'HALT' : 'ENGAGE'}
          </button>
          
          <button 
            onClick={onTriggerAlert}
            className="bg-white text-black font-black py-2.5 px-6 rounded-2xl text-[10px] tracking-widest hover:bg-red-500 hover:text-white transition-colors active:scale-95"
          >
            PANIC_OVERRIDE
          </button>
        </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 min-h-0">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-end space-x-6">
                  <span className={`text-9xl font-black tracking-tighter leading-none ${globalThreat === ThreatLevel.CRITICAL ? 'text-red-500' : 'text-white'}`}>
                    ...
                    <span className="text-xl ml-2 text-white/20 uppercase font-black">m</span>
                  </span>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col space-y-4 min-h-0">
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard title="Fatigue" value={`${(stats.fatigue * 100).toFixed(0)}%`} color={stats.fatigue > 0.7 ? 'text-red-500' : 'text-white'} />
            <StatCard title="Status" value={isEmergencyAlarm ? "DANGER" : (errorStatus ? "LIMIT" : "CLEAR")} color={isEmergencyAlarm || errorStatus ? 'text-red-500' : 'text-emerald-500'} />
          </div>
          ...
          <div className="h-44 bg-black/40 rounded-[2rem] border border-white/10 p-5 flex flex-col min-h-0">
